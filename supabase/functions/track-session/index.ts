...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!validation.valid) {
      console.warn(`HMAC validation failed for track-session: ${validation.error}`);
      return jsonResponse({ error: validation.error }, 401);
    }

    if (validation.userId !== telegram_id) {
      console.warn(`User ID mismatch in track-session: expected ${validation.userId}, got ${telegram_id}`);
